...
if [ $OPTION = 'Run' ]; then
    ...
    if [ $SYSTEM = 'WSL2' ]; then
        SIMULATOR_COMMANDS="docker run --name sarax_container -it -v /tmp/.X11-unix:/tmp/.X11-unix -v /mnt/wslg:/mnt/wslg -v /usr/lib/wsl:/usr/lib/wsl --device=/dev/dxg --device /dev/dri/card0 --device /dev/dri/renderD128 -e DISPLAY=$DISPLAY -e WAYLAND_DISPLAY=$WAYLAND_DISPLAY -e XDG_RUNTIME_DIR=$XDG_RUNTIME_DIR -e PULSE_SERVER=$PULSE_SERVER --gpus all $IMAGE_ID bash"
    fi
